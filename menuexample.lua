...
mainMenu = NativeUI.CreateMenu("Chase", "~b~Menu")
...
function AddRepair(menu)
    local newitem = NativeUI.CreateItem("Repair", "Repair your vehicle")
    menu:AddItem(newitem)  
end

function AddRecording(menu)
    local newitem = NativeUI.CreateItem("Recording Start", "Start the in-game recording")
    menu:AddItem(newitem)
end

function AddRecordingStop(menu)
    local newitem = NativeUI.CreateItem("Recording Stop", "Stop the in-game recording")
    menu:AddItem(newitem)
...
function AddReset(menu)
    local newitem = NativeUI.CreateItem("Reset", "Reset the Chase Game")
    menu:AddItem(newitem)
end

function AddResetTimer(menu)
    local newitem = NativeUI.CreateItem("Reset Timer", "Reset the Timer for Chase Game, everything else stays the same")
    menu:AddItem(newitem) 
end

function BindItemSelect(menu)


 menu.OnItemSelect = function(sender, item, index)
    print(index)
    if index == 1 then
        TriggerEvent("chase:localrepair")
    end
   if index == 2 then
        TriggerEvent("chase:startrecord")
    end 
    if index == 3 then
        TriggerEvent("chase:stoprecord")    
    end
    if index == 4 then
        TriggerEvent("chase:reset")       
    end
    if index == 5 then
        TriggerEvent("chase:resettimer")      
    end
end
end
...
AddRepair(mainMenu)
AddRecording(mainMenu)
AddRecordingStop(mainMenu)
AddReset(mainMenu)
AddResetTimer(mainMenu)


BindItemSelect(mainMenu)
